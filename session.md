...
43. **Phase 12 (indicator library + picker) built (2026-09-11).** API: `api/app/services/indicators.py` is now a registry (`REGISTRY: dict[id, IndicatorSpec]`, 20 entries: 10 overlays sma/ema/bb/vwap/psar/ichimoku/keltner/donchian/sr/pivot, 10 panes rsi/macd/stoch/adx/atr/cci/obv/willr/mfi/roc) with `Param(name, default, min, max, integer)`, `outputs`, `reference_lines`, `description`, `formula`, `compute(df, **params)`; helpers `wilder` (seeded with the simple mean, textbook tables), `true_range`; RSI/ADX seed over bars 1..N (bar 0 has no change). Request tokens `id[:p1-p2-…]` (`parse_token` fills defaults, validates ranges, MACD fast<slow; `parse_tokens` dedupes on the canonical token, ≤ 8 else 422; empty = `DEFAULT_TOKENS` ema:10/30/60/90, bb:20-2, sr). `GET /indicators/catalog` → `{indicators, defaults, max_per_request}`; `GET /indicators/{ticker}?ind=…` → `{candles, series: {token: {id, kind, params, outputs}}, levels}` (**`ema`/`bollinger` fields are gone**; `levels` only when `sr` is requested). Crypto works through the same route (Coinbase candles). `api/scripts/export_catalog.py` writes `src/test/fixtures/indicator-catalog.json`; `test_catalog_fixture_is_current` fails when stale. Tests: `test_indicators.py` 61 (hand values for wilder/TR/RSI/MACD/OBV/ROC/pivots/VWAP, invariants for the rest, every registry entry computes its outputs, token parsing), router tests for defaults/tokens/catalog/422. Frontend: `src/lib/indicators.ts` (tokens, `canonicalToken`/`normaliseTokens` against the catalog, `parseTokens` for persisted lists, `fromLegacy` for old `ov=`/widget `overlays`, `tokenLabel` "RSI 14"/"MACD 12/26/9"), selection saved like portfolios (`src/stores/indicators.ts` local, `indicator-repo.ts` + `use-indicator-selection.ts` cloud with 1 s debounce and first-sign-in seeding; **draft is React state, not the query cache** — fake timers never flush TanStack's notifier), Supabase `indicator_settings` (user_id pk, tokens jsonb ≤ 8, RLS; migration `20260911180000_indicator_settings.sql` applied via MCP; pgTAP `0003_indicator_settings.test.sql` unrun), `IndicatorPicker` (button + `role=dialog` popover or `inline` for widget settings; chips with number inputs that commit on blur/Enter, remove buttons, grouped native `<select>` to add, cap 8, Reset), `PriceChart({data})` draws whatever `data.series` holds — overlays on pane 0, each oscillator in its own lightweight-charts v5 pane (`addSeries(…, paneIndex)`, stretch 3:1, `removePane` on rebuild), MACD `hist` as a histogram coloured by sign, reference lines per oscillator, SAR as dots, `BandFill` series primitive fills upper→lower (bb/keltner/donchian) and the Ichimoku cloud (closes the "Bollinger fill" backlog item), legend lists every series; Charts tab uses the saved selection + picker (URL `ind=`/legacy `ov=` applied once then removed), Chart widget config `overlays` → `indicators` (migrated in `parseWidgetConfig`) with the inline picker in settings, Crypto coin detail shares the selection. `chart-theme` palette is now a generic `series[]` list. Tests: frontend 203 / 39 files (`indicators.test.ts`, `indicator-picker.test.tsx` 5, `use-indicator-selection.test.tsx` 4, charts-page/price-chart rewritten, chart-mock gained panes/primitives). e2e: chart step now removes BB and adds RSI + MACD via the picker, crypto step adds RSI on BTC-USD — 3/3 locally (one transient Yahoo search miss on the first run).
44. **Phase 13 (FAQ + legal disclosure) built (2026-09-11).** `api/scripts/export_endpoints.py` reads the running app's OpenAPI schema and a hand-written `NOTES` map (purpose, cache TTL keys, rate-limit setting name per path) to write `src/content/endpoints.json` (method, path, tag, purpose, cache entries with live seconds from `Settings`, rate limit); `test_endpoints_fixture_is_current` fails if the file is stale or a route is missing from `NOTES`, mirroring the Phase 12 catalog-fixture pattern. Frontend content lives as typed data: `src/content/faq.ts` (six sections — what this is, data sources and delays, model assumptions, API endpoints [rendered as a table from `endpoints.json`, not hand-typed], accounts and privacy, limits; delay numbers are read live via a `ttl()` lookup into the generated fixture and formatted by `seconds()`, so "cache each quote for 1 minute" etc. can never drift from `Settings`) and `src/content/disclaimer.ts` (six sections: no investment advice, invest at your own risk, no warranty on data, AI-generated content, limitation of liability, acceptance — a first-draft disclosure, **not reviewed by a lawyer**, flagged as such in the file). `FaqPage` (`src/features/faq/`) renders the sections with an anchor table of contents, the generated endpoint table, and the live indicator catalog (`GET /indicators/catalog`) as a formula/defaults table — closing the loop so indicator docs can't drift either. `DisclaimerPage` (`src/features/legal/`) renders the disclosure. Every page now carries a footer (`AppShell`: short disclosure line + links to `/disclaimer` and `/faq` + data-source credit) and a one-time `AcknowledgementBar` ("I understand", remembered in `localStorage` under `stock-tool.disclaimer-ack`, never shown again once accepted). The sentiment panel's fixed disclaimer text now links to `/disclaimer`. Nav gained "FAQ"; `/disclaimer` is reachable from the footer only (not primary nav) to keep the bar's own link meaningful. Tests: API +1 (`test_endpoints_fixture_is_current`); frontend 212 / 41 files (`faq-page.test.tsx` 5, `legal.test.tsx` 3 covering the disclaimer page, footer and acknowledgement bar, and the sentiment link) — caught and fixed a singular/plural bug in `seconds()` (1 minute was rendering as "1 minutes") and two ambiguous test queries (multiple "FAQ" links between nav and footer; multiple "period 14" cells across indicators) during the pass. e2e: one new step (accept the bar, visit FAQ, check the generated tables, follow the footer to the disclaimer, reload and confirm the bar stays dismissed) — 3/3 locally alongside the search and crypto steps; full suite 7/8 with one transient Yahoo-search timeout on `portfolio tab` that passed alone on retry (unrelated to this phase).
...
### Phase 12 — Indicator library + picker
...
**Next: "Expansion plan 2" (Phases 12–14: indicator library + picker, FAQ + disclosure, polish) — approved
2026-09-11 (decisions recorded in that section); Phase 12 built (note 43) and Phase 13 built (note 44),
see their checklists for deploy status; Phase 14 (polish backlog) is next after Phase 13 deploys.**
...
**Pending from the user:** Render Manual Deploy + Lovable Publish for Phases 12 and 13 together (see
their checklists); the disclaimer text in `src/content/disclaimer.ts` should get a lawyer's review
before the site is treated as public-facing.
...
1. The list of 20 stands, no swaps.
2. Disclosure: page + footer line + one-time acknowledgement bar.
3. Indicator selection is saved to the account like portfolios (local store signed out, Supabase
   row signed in, seeded from local on first sign-in); chart widgets keep their own per-widget list.
