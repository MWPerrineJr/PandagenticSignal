...
describe('FanChart', () => {
  it('draws two bands, a median line and axis ticks, with a table alternative', async () => {
    ...
    expect(within(svg).getByText('0y')).toBeInTheDocument()
    expect(within(svg).getByText('10y')).toBeInTheDocument()
  ...
  it('handles a one-year horizon and a sparse series', () => {
    ...
    expect(within(screen.getByRole('img', { name: /median and 5th/i })).getByText('1y')).toBeInTheDocument()
  })
})
